...
namespace SCJMapper_V2.SC
{
    public class DProfileReader
    {
        ...
        public void Actions()
        {
            // Include all actions that have any kind of input binding (keyboard, mouse, joystick, gamepad)
            // This ensures newer actions without default keyboard bindings are still available
            actions = maps
        ...
        /// <summary>
        /// Get all actions including unbound ones for complete functionality list
        /// </summary>
        ...
        /// <summary>
        /// Get unbound actions (actions without any default input bindings)
        /// </summary>
        public Dictionary<string, Action> GetUnboundActions()
        {
            return maps
                ...
                .Where(x => string.IsNullOrWhiteSpace(x.Keyboard) &&
                            ...
                            string.IsNullOrWhiteSpace(x.Joystick) &&
                            string.IsNullOrWhiteSpace(x.Gamepad))
                .Where(x => !string.IsNullOrWhiteSpace(x.UILabel)) // Must have a display name
                .ToDictionary(x => x.Name, x => x);
        }
        ...
        public void CreateCsv(bool enableCsvExport)
        {
            ...
            try
            {
                    ...
                    new StreamWriter(Path.Combine(TheUser.FileStoreDir, "mousebindings.csv")))
                {
                    ...
                    var mouseActions = maps
                        .SelectMany(x => x.Value.Actions.Values)
                        .Where(x => !string.IsNullOrWhiteSpace(x.Mouse))
                        .ToDictionary(x => x.Name, x => x);


                    foreach (var action in mouseActions.OrderBy(x => x.Value.MapUILabel)
                        .ThenBy(x => x.Value.UILabel))
                    {
                        var csvline = action.Value.MapUICategory + "\t" + action.Value.MapUILabel + "\t" +
                                      action.Value.MapName + "\t";
                    ...
                    new StreamWriter(Path.Combine(TheUser.FileStoreDir, "joystickbindings.csv")))
                {
                    ...
                    var joystickActions = maps
                        .SelectMany(x => x.Value.Actions.Values)
                        .Where(x => !string.IsNullOrWhiteSpace(x.Joystick))
                        .ToDictionary(x => x.Name, x => x);


                    foreach (var action in joystickActions.OrderBy(x => x.Value.MapUILabel)
                        .ThenBy(x => x.Value.UILabel))
                    {
                        var csvline = action.Value.MapUICategory + "\t" + action.Value.MapUILabel + "\t" +
                                      action.Value.MapName + "\t";
